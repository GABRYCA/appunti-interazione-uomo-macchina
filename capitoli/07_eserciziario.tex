...
\begin{enumerate}
    ...
    Per calcolare il livello associato, è \textbf{fondamentale} calcolare la differenza tra il segnale in ingresso e la tensione minima di fondo scala ($V_{min}$), per poi dividere per il passo $\Delta$.
    Questa formula generale garantisce il corretto funzionamento anche con segnali bipolari o con offset:
    $$Livello = \text{round}\left( \frac{V_{in} - V_{min}}{\Delta} \right)$$

    Nel nostro caso specifico ($V_{min} = 0\text{V}$):
...
\fcolorbox{black}{yellow!10}{
    \begin{minipage}{\linewidth}
        \textbf{Attenzione (Errore Comune):} Usare la formula semplificata $\text{round}(V_{in} / \Delta)$ funziona \textbf{solo} se $V_{min} = 0\text{V}$.

        Se ad esempio avessimo avuto un ADC con range da $\mathbf{-5\text{V}}$ a $\mathbf{+5\text{V}}$ ($\Delta = \frac{10}{255} \approx 0.0392\text{V}$) e lo stesso ingresso di $2.15\text{V}$, il calcolo errato darebbe:
        $$\text{round}\left(\frac{2.15}{0.0392}\right) = 55 \quad \text{\color{red}{(Sbagliato, manca fondoscala negativo!)}}$$

        Il calcolo corretto applicando la formula generale è:
        $$\text{round}\left(\frac{2.15 - (-5)}{0.0392}\right) = \text{round}\left(\frac{7.15}{0.0392}\right) = \mathbf{182} \quad \text{\color{green!50!black}{(Corretto!)}}$$
    \end{minipage}
}
