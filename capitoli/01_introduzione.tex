...
\begin{figure}[h!]
    \centering
    \begin{tikzpicture} [
        every node/.style={draw, rectangle, rounded corners, minimum height=1.2cm, align=center, text width=2.5cm, fill=green!10},
        arrow/.style={->, thick, >=Stealth}
    ]
        \node (n1) {Manuale \\ (Batch)};
        \node[right=1cm of n1, fill=yellow!10] (n2) {CLI \\ (Command Line)};
        \node[right=1cm of n2, fill=orange!10] (n3) {GUI \\ (Interfacce Grafiche)};
        \node[below=1.5cm of n3, fill=red!10] (n4) {Web \& \\ Mobile};
        \node[left=1cm of n4, fill=purple!10, text width=3cm] (n5) {Multimodale / \\ Intelligente};
        ...
        \draw[arrow] (n4) -- (n5);
    \end{tikzpicture}
    \caption{Schema evolutivo delle Interfacce Uomo-Macchina.}
\end{figure}
\vspace{0.5cm}
